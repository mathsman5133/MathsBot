    ...
    async def paginate(self):
        ...
        while self.paginating:
            ...
            try:
                ...
                if counter == 0:
                    counter += 1
                    c = await self.channel.send('I need `manage_message` permissions to remove your reactions!')
                    await asyncio.sleep(5)
                    await c.delete()
...
class HelpPaginator(Pages):
    ...
    async def from_command(cls, ctx, command):
        prefix = cleanup_prefix(ctx.bot, ctx.prefix)
        # return self
        count = 0
        ...
        # embed = discord.Embed(title=f'Command Name: {command.qualified_name}', colour=discord.Colour.dark_orange())
        #
        ...
        self = cls(ctx, entries)
        self.title = f'{command.signature}'
